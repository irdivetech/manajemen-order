...
@section('page-header')
    <h1>Laporan Modal Belanja (HPP)</h1>
    <p class="page-sub">Rincian estimasi biaya produksi</p>
@endsection
...
{{-- ── Key Metrics ── --}}
<div class="m-card mb-3" style="background:linear-gradient(135deg,#dc2626,#991b1b); border:none;">
    <div class="m-card-body text-white">
        <div class="text-xs fw-6 text-uppercase mb-1" style="color:rgba(255,255,255,0.8); letter-spacing:0.05em;">Total HPP / Modal Belanja</div>
        <div style="font-size:1.5rem; font-weight:700;">Rp {{ number_format((float) $totalHpp, 0, ',', '.') }}</div>
    </div>
</div>
    ...
    <div class="col-12">
        <div class="m-card h-100 border-primary">
            <div class="m-card-body py-3">
                <div class="text-xs text-muted fw-6 mb-1">Pesanan Terkait</div>
...
<div class="d-flex justify-content-between align-items-center mb-2">
    <div class="section-title mb-0">Rincian Belanja</div>
    <a href="{{ route('hpp.export', request()->all()) }}" class="btn btn-outline-success btn-sm d-flex align-items-center gap-1">
...
<div class="m-card">
    <div class="m-card-body p-0">
        ...
        <a href="{{ route('orders.show', $order) }}" class="m-list-item d-flex align-items-center gap-3 text-decoration-none text-dark border-bottom">
            <div class="flex-grow-1 min-w-0">
                <div class="d-flex justify-content-between align-items-start mb-1">
                    <span class="fw-7 text-sm">{{ $order->order_number }}</span>
                    <span class="fw-7 text-sm text-danger">Rp {{ number_format($order->total_cost, 0, ',', '.') }}</span>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center mt-2">
                    ...
                    <span class="badge bg-light text-dark border">{{ $order->material }}</span>
                </div>
            </div>
        </a>
        @empty
...
<div class="offcanvas offcanvas-bottom" tabindex="-1" id="filterOffcanvas" style="border-radius:20px 20px 0 0; min-height:50vh; height:auto;">
    ...
    <div class="offcanvas-body px-4" style="padding-bottom: calc(var(--nav-height, 64px) + 1.5rem) !important;">
                <form action="{{ route('hpp.index') }}" method="GET" id="reportFilterForm">
